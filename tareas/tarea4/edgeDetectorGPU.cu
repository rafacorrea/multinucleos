...
__global__ void convolucionGPU(int *mask1, int* mask2, unsigned char *img, unsigned char *res, int M, int N)
{
    ...
    i += 1; //para que empieze en N+1 y M+1
    j += 1;
   
    if (i*M+j >= M*N) //para asegurarse que no accese a memoria que no existe...
        return;
    ...
    int sum = abs(temp1) + abs(temp2); //quitar negativos...
    sum = sum > 255 ? 255 : sum; //asegurarse que no se pase de 255
    sum = sum < 0 ? 0 : sum;
    res[i*M+j] = static_cast<unsigned char> (sum); //cast a uchar
   
}


void prewittEdge(const Mat &input, Mat &output, int numthreads)
{
	...
	dim3 bloques((input.rows - 2)/ numthreads + 1, (input.cols - 2)/numthreads + 1);
...
int main(int argc, char *argv[])
{
    ...
    prewittEdge(input, output, threads);
